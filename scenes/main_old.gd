...
func set_current_player(side):
	if side ==1:
		if turn_queue_1.size() > 0:
			...
			current_player_1.get_node("Circle").visible = true
		...
		if turn_queue_2.size() > 0:
			...
			current_player_2.get_node("Circle").visible = true
...
func end_turn(side):
	if side == 1:
		...
		wait_1 = true
		current_player_1.hold_time = 0
		current_player_1.get_node("Circle").visible = false
	else:
		clear_bar(bar_2)
		wait_2 = true
		current_player_2.hold_time = 0
		current_player_2.get_node("Circle").visible = false
